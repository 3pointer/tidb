...
func RewriteFileKeys(file AppliedFile, rewriteRules *RewriteRules) (startKey, endKey []byte, err error) {
	...
	if startID == endID {
		...
		if rewriteRules != nil && rule == nil {
			...
			if rule == nil {
				log.Error("cannot find start key match table with file", zap.Any("file", file), zap.Any("startKey", file.GetStartKey()),
					zap.Any("startID", startID), zap.Any("endID", endID), zap.Any("rule", rewriteRules))
